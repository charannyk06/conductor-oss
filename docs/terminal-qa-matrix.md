...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Local attach | macOS or Linux desktop on loopback | token + backend ttyd facade websocket | connect under 150 ms, usable prompt under 2 s | native typing, stable scroll, no duplicate input or transport fallback | | Pending | |
| Local refresh restore | same desktop session after reload | token refresh + backend ttyd facade websocket | reconnect under 2 s | prompt and recent scrollback survive refresh | | Pending | |
