...
impl TerminalHostRegistry {
    ...
    pub(crate) async fn ensure_host(
        ...
        let handle = Arc::new(LiveSessionHandle {
            ...
            retained_kill_txs: tokio::sync::Mutex::new(Vec::new()),
            retained_input_txs: tokio::sync::Mutex::new(Vec::new()),
        });
    ...
    pub(crate) async fn detach_runtime(&self, handle: &Arc<LiveSessionHandle>) {
        *handle.resize_tx.write().await = None;
        ...
        if let Some(input_tx) = handle.input_tx.write().await.take() {
            handle.retained_input_txs.lock().await.push(input_tx);
        }
        if let Some(kill_tx) = handle.kill_tx.lock().await.take() {
            handle.retained_kill_txs.lock().await.push(kill_tx);
        }
        let mut tracking = handle.terminal_persistence.lock().await;
        let now = Instant::now();
    ...
    pub(crate) async fn take_kill_senders(
        &self,
        handle: &Arc<LiveSessionHandle>,
    ) -> Vec<oneshot::Sender<()>> {
        let mut senders = Vec::new();
        if let Some(kill_tx) = handle.kill_tx.lock().await.take() {
            senders.push(kill_tx);
        }
        senders.append(&mut *handle.retained_kill_txs.lock().await);
        senders
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn registry_preserves_runtime_senders_across_multiple_detach_cycles() {
        let registry = TerminalHostRegistry::default();
        let handle = registry.ensure_host("session-1", None).await;

        let (input_tx_a, mut input_rx_a) = mpsc::channel::<ExecutorInput>(1);
        let (kill_tx_a, mut kill_rx_a) = oneshot::channel();
        registry
        ...
        let (kill_tx_b, mut kill_rx_b) = oneshot::channel();
        registry
        ...
        let drained = registry.take_kill_senders(&handle).await;
