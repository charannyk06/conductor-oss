	...
	push(chunk: Buffer): PtySubprocessFrame[] {
		...
		while (offset < chunk.length) {
			if (this.payload === null) {
				...
				const validTypes = Object.values(PtySubprocessIpcType).filter(
					(v): v is number => typeof v === "number",
				);
				if (!validTypes.includes(rawType)) {
					// Reset decoder state and skip this frame
					this.headerOffset = 0;
					this.payload = null;
				...
				const type = rawType as PtySubprocessIpcType;
