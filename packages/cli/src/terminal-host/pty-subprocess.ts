...
// CRITICAL: Use array buffering to avoid O(n^2) string concatenation.
...
function handleKill(payload: Buffer): void {
	const rawSignal = payload.length > 0 ? payload.toString("utf8") : "SIGTERM";
	const signal = VALID_SIGNALS.has(rawSignal) ? rawSignal : "SIGTERM";
...
function handleSignal(payload: Buffer): void {
	const rawSignal = payload.length > 0 ? payload.toString("utf8") : "SIGINT";
	const signal = VALID_SIGNALS.has(rawSignal) ? rawSignal : "SIGINT";
