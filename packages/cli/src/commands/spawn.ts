...
export function registerSpawn(program: Command): void {
  program
    ...
    .argument(
      "[issueOrPrompt]",
      "Issue identifier (#42, INT-123) or inline prompt"
    )
    .option(
      "--agent <name>",
      "Override agent plugin (e.g. claude-code, codex, gemini, amp, hermes, cursor-cli, opencode, pi, droid, qwen-code, ccr, github-copilot, letta, openai-codex, google-gemini, open-code)"
    )
    .option("--model <name>", "Override model (e.g. o4-mini, claude-opus-4-6)")
    .option(
      "--reasoning-effort <level>",
      "Override reasoning effort when the target CLI supports it"
    )
    ...
    .action(
      async (
        project: string,
        issueOrPrompt: string | undefined,
        opts: SpawnOptions
      ) => {
        ...
        try {
          ...
          const { session } = await apiCall<SessionResponse>(
            ...
            "/api/sessions/spawn",
            {
              ...
              model: opts.model,
              reasoningEffort:
                opts.reasoningEffort?.trim().toLowerCase() || undefined,
              useWorktree: true,
            }
          );
          ...
          console.log(
            `  Worktree: ${chalk.dim(sessionWorktree(session) ?? "-")}`
          );
          if (session.branch) {
          ...
          console.log(
            `  Terminal: ${chalk.dim(
              "Open the session in the dashboard terminal"
            )}`
          );
          console.log();
