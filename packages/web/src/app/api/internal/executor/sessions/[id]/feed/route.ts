...
import { authorizeInternalExecutorRequest, getLocalExecutionFeed } from "@/lib/executionBackend";
...
export async function GET(
  ...
  try {
    const payload = await getLocalExecutionFeed(sessionId, lines);
    return NextResponse.json(payload);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to load execution feed";
