...
test("GET falls back to HTTP polling when no remote backend websocket endpoint is available", async () => {
  resetEnv();
  process.env.CONDUCTOR_BACKEND_URL = "http://127.0.0.1:4749";

  global.fetch = (async () => {
    throw new Error("terminal token lookup should not run for HTTP polling fallback");
  }) as typeof fetch;
