...
import { DispatcherChatDock } from "@/components/dispatcher/DispatcherChatDock";
import {
  buildModelSelection,
...
export function ProjectDispatcherPanel({
  ...
  const handleCreate = useCallback(async (forceNew: boolean) => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => null);
      if (!response.ok) {
        throw new Error(payload?.error ?? "Failed to start dispatcher chat");
      }
      ...
      setError(err instanceof Error ? err.message : "Failed to start dispatcher chat");
    } finally {
      setCreating(false);
    }
