/**
 * Shared utility functions for Conductor plugins and core.
 */

import { open, stat } from "node:fs/promises";

// Re-export shellEscape from types for convenience
export { shellEscape } from "./types.js";

/**
 * Validate that a URL starts with http:// or https://.
 * Throws with a descriptive error including the label if invalid.
 */
...
async function readLastLine(filePath: string): Promise<string | null> {
  ...
  try {
    ...
    while (pos > 0) {
      ...
      // Convert all accumulated bytes to string at once (safe for multi-byte UTF-8)
